...
func (rc *ResponsesClient) Generate(ctx context.Context, req *api.Request) (out *api.Response, err error) {
	...
	for _, item := range resp.Output {
		...
		case responses.ResponseOutputMessage:
			msg := api.Output{
				ID:     variant.ID,
				Role:   api.RoleAssistant, // Always assistant role for response messages.
			...
			if len(annotations) > 0 {
				msg.Citations = make([]api.Citation, 0, len(annotations))
	...
	if len(out.Output) == 0 && len(out.ToolCalls) == 0 {
		if text := resp.OutputText(); text != "" {
			out.Output = append(out.Output, api.Output{
...
// Create a responses.ResponseNewParams body from a api.Request.
//
// Handling notes follow.
//
// - Background: always false for Horizon
// - Store: always false for Horizon
// - Instructions: currently unused in Horizon
// - ParallelToolCalls: enabled when tools are advertised; Horizon performs calls directly
// - PreviousResponseID: currently unused in Horizon
// - User: deprecated, and so unused in Horizon
// - ContextManagement: currently unused in Horizon
// - Conversation: currently unused in Horizon
// - Metadata: currently unused in Horizon
// - Prompt: currently unused in Horizon (Horizon performs prompt rendering directly)
...
// - Reasoning: Horizon uses ReasoningEffort and ReasoningSummary parameters.
...
func ResponsesBody(req *api.Request) (body responses.ResponseNewParams, err error) {
...
func ResponsesMessages(in prompts.Prompts, attachments []*api.Attachment) (out responses.ResponseNewParamsInputUnion, err error) {
	...
	for _, msg := range in {
		...
		default:
			return out, fmt.Errorf("unsupported message role: %v", msg.Role)
		}
...
func ResponseStatusType(status responses.ResponseStatus, incompleteReason string) api.OutputType {
	switch status {
...
func ResponsesAttachments(attachments []*api.Attachment) (parts responses.ResponseInputMessageContentListParam, err error) {
	var uri string
