/*
Package mime provides functionality for working with media types and file extensions
specifically for text, audio, images, and video in an LLM context. This package is not
meant to be a comprehensive MIME type library, but rather to expose short constants
and unified handling for content and media types being handled by Horizon.

// TODO: move this package to go.rtnl.ai/x/mime
*/
package media
...
var (
	ErrEmptyMediaType   = errors.New("empty media type")
...
func Parse(s string) (out MediaType, err error) {
...
// MediaType represents a media type value and any optional parameters, per RFC 1521.
// Media types are the values in Content-Type and Content-Disposition headers (RFC 2183).
...
func (m Type) Modality() modality.Modality {
	switch {
	case m.IsUnknown():
		return modality.ZeroModality
	case m.IsText():
...
var iconMap = map[Type]string{
	...
	UnknownMediaType: "far fa-fw fa-file",
}
...
func (m Type) Icon() string {
	...
	return iconMap[UnknownMediaType]
}
